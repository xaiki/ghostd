...
func (l *listener6) HandleMsg6(buf []byte, oob *ipv6.ControlMessage, peer *net.UDPAddr) {
	...
	case dhcpv6.MessageTypeRequest, dhcpv6.MessageTypeConfirm, dhcpv6.MessageTypeRenew,
		...
		resp, err = dhcpv6.NewReplyFromMessage(msg)
	case dhcpv6.MessageTypeDecline, dhcpv6.MessageTypeInformationRequest:
		resp = &dhcpv6.Message{MessageType: dhcpv6.MessageTypeReply, TransactionID: msg.TransactionID}
	default:
...
func (l *listener6) Serve() error {
	...
	for {
		...
		select {
		case l.slots <- struct{}{}:
			go func() { defer func() { <-l.slots }(); l.HandleMsg6(b[:n], oob, peer.(*net.UDPAddr)) }()
		default:
			bufpool.Put(&b)
		}
	}
...
func (l *listener4) Serve() error {
	...
	for {
		...
		select {
		case l.slots <- struct{}{}:
			go func() { defer func() { <-l.slots }(); l.HandleMsg4(b[:n], oob, peer.(*net.UDPAddr)) }()
		default:
			bufpool.Put(&b)
		}
	}
