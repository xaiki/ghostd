...
type listener6 struct {
	*ipv6.PacketConn
	...
	guard    func(dhcpv6.DHCPv6, net.Addr) func()
	slots    chan struct{}
}
...
type listener4 struct {
	*ipv4.PacketConn
	...
	guard    func(*dhcpv4.DHCPv4, net.Addr) func()
	slots    chan struct{}
}
...
func listen4(a *net.UDPAddr, supplied net.PacketConn) (*listener4, error) {
	var err error
	l4 := listener4{}
	udpConn := supplied
	if udpConn == nil {
...
func listen6(a *net.UDPAddr, supplied net.PacketConn) (*listener6, error) {
	l6 := listener6{}
	var err error
	udpconn := supplied
	if udpconn == nil {
...
type Options struct {
	...
	Guard4       func(*dhcpv4.DHCPv4, net.Addr) func()
	Guard6       func(dhcpv6.DHCPv6, net.Addr) func()
}

func Start(config *config.Config, options Options) (*Servers, error) {
	...
	if config.Server6 != nil {
		...
		for _, addr := range config.Server6.Addresses {
			var l6 *listener6
			l6, err = listen6(&addr, options.Conn6)
			if err != nil {
				goto cleanup
			}
			l6.handlers = handlers6
			l6.guard = options.Guard6
			l6.slots = make(chan struct{}, 64)
	...
	if config.Server4 != nil {
		...
		for _, addr := range config.Server4.Addresses {
			var l4 *listener4
			l4, err = listen4(&addr, options.Conn4)
			if err != nil {
				goto cleanup
			}
			l4.handlers = handlers4
			l4.guard = options.Guard4
			l4.slots = make(chan struct{}, 64)
